drop function if exists public.book_public_appointment(text,date,time,text,text,text,text,text);
...
begin
  ...
  if v_patient_id is null then
    ...
    insert into public.patients (
      doctor_id, patient_number, full_name, mobile, email, age, gender,
      current_reason, next_action, expected_followup_date, status
    )
    values (
      v_doctor_id, v_patient_number, p_patient_name, p_mobile, p_patient_email, p_age, p_gender,
...
grant execute on function public.book_public_appointment(text,date,time,text,text,text,text,text,text) to anon, authenticated;
